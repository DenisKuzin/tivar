...
def get_trades(ticker_id, start, stop):
    '''
    ...
    csv = get('http://sberbank-am.ru/visible/chart/getCsv?fund=%s&dateFrom=%s&monthFrom=%s&yearFrom=%s&dateTo=%s&monthTo=%s&yearTo=%s' % (
        ticker_id, start.day, start.month, start.year, stop.day, stop.month, stop.year))
    trades = []
    for line in csv.text.split('\n')[3:]:
        splited_line = line.split(';')
        if len(splited_line) == 3:
            trades.append({'time': datetime.strptime(splited_line[0], '%d-%m-%y'), 'price': float(splited_line[1].replace(',', '.')), 'NAV': float(splited_line[2])})
    trades.reverse()
    return trades
